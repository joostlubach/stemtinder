...
class FileDriverTest extends DoctrineTestCase
{
    ...
    public function testGetElementFromFile()
    {
        ...
        $locator->expects($this->once())
                ...
                ->will($this->returnValue(dirname(__FILE__) . '/_files/stdClass.yml'));
    ...
    {
        $driver = new TestFileDriver(dirname(__FILE__) . '/_files', '.yml');
        $this->assertTrue($driver->isTransient('stdClass2'));
        $this->assertFalse($driver->isTransient('stdClass'));
    }
    ...
    private function newLocator()
    {
        ...
        $locator->expects($this->any())->method('getPaths')->will($this->returnValue(array(dirname(__FILE__) . "/_files")));
        return $locator;
    }
}
